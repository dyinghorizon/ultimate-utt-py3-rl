...
from ultimate_ttt_rl.player import RandomTTTPlayer, RLTTTPlayer
from ultimate_ttt_rl.ultimateplayer import RandomUTTTPlayer, RLUTTTPlayer
from ultimate_ttt_rl.learning import NNUltimateLearning, TableLearning
from ultimate_ttt_rl.plotting import drawXYPlotByFactor
...
def playTTTAndPlotResults():
    ...
    plotValues = {'X Win Fraction': list(zip(range(numberOfSetsOfGames), [x[0] for x in results])),
                  'O Win Fraction': list(zip(range(numberOfSetsOfGames), [x[1] for x in results])),
                  'Draw Fraction': list(zip(range(numberOfSetsOfGames), [x[2] for x in results]))}
    drawXYPlotByFactor(plotValues, 'Number of Sets (of 100 Games)', 'Fraction', 
